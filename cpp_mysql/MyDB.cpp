...
bool MyDB::initDB(string host, string user, string pwd, string db_name) {
    ...
    mysql = mysql_real_connect(&mysql, host.c_str(), user.c_str(), pwd.c_str(),
            db_name.c_str(), 3306, NULL, 0);
    if (&mysql == NULL){
        cout << "Error: 初始化错误" << mysql_error(&mysql);
        exit(1);
    }
    return true;
}
...
bool MyDB::exeSQL(string sql) {
    ...
    if (mysql_query(&mysql, sql.c_str())){
        ...
        if (result){  // 返回了结果集
            int num_fields = mysql_num_fields(result);  // 获取结果集中总共的字数
